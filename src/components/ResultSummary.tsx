...
export default function ResultSummary({ type, result }: ResultSummaryProps) {
    ...
    const generateMortgageSummary = (result: any) => {
        ...
        const affordabilityNote = monthlyPayment > 2000 ?
            ...
            "This monthly payment falls within a moderate range for many US households.";

        return `Based on your inputs, your estimated monthly mortgage payment is $${monthlyPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}. Over the life of your loan, you'll pay a total of $${totalPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}, which includes $${principal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} in principal and $${totalInterest.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} in interest. This means ${interestPercentage}% of your total payments will go toward interest charges. ${affordabilityNote} Consider that US mortgage rates fluctuate based on Federal Reserve policy, your credit score, and market conditions. A difference of just 0.5% in your interest rate can save or cost you tens of thousands of dollars over a 30-year mortgage. If you can afford extra payments, even small additional amounts can significantly reduce your total interest and shorten your loan term. Many US homeowners refinance when rates drop to lower their monthly payments or switch from an ARM to a fixed-rate mortgage for payment stability.`;
    };
    ...
    const generateLoanSummary = (result: any) => {
        ...
        return `Your calculated monthly payment is $${monthlyPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}. Over the full loan term, you'll repay a total of $${totalPayment.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}, consisting of your original $${principal.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} principal plus $${totalInterest.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} in interest charges. This represents a ${costRatio}% interest cost relative to the amount borrowed. ${rateAdvice} In the US, personal loan rates typically range from 5% to 36% depending on your credit score and the lender. Auto loans generally offer lower rates (3-10%) because the vehicle serves as collateral. Student loans may have rates as low as 3-7% for federal loans. Before accepting any loan, compare offers from multiple lenders, as rates and terms can vary significantly. Also consider the total cost of the loan, not just the monthly payment - a longer term means lower monthly payments but much higher total interest paid.`;
    };
    ...
    const generatePropertyTaxSummary = (result: any) => {
        ...
        const affordabilityNote = annualTax > 5000 ?
            "This is a substantial property tax bill. In high-tax states like New Jersey, Illinois, or Connecticut, property taxes can rival or exceed mortgage payments for some homeowners." :
            "This property tax amount is moderate compared to national averages.";

        return `Based on your home's assessed value and local tax rate, your estimated annual property tax is $${annualTax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}, which breaks down to approximately $${monthlyTax.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} per month. ${affordabilityNote} Property taxes are a significant ongoing cost of homeownership in the US and should be carefully considered when budgeting for a home purchase. These taxes fund essential local services including public schools (typically 40-60% of property tax revenue), police and fire departments, road maintenance, libraries, and parks. The quality of these services often correlates with property values, creating a cycle where higher taxes support better services, which in turn maintain or increase home values. Property tax rates vary enormously across the country - Hawaii has the lowest effective rate at about 0.28%, while New Jersey has the highest at around 2.49%. Within states, rates can vary significantly by county and municipality. Many jurisdictions offer exemptions or reduced rates for seniors, veterans, or primary residences (homestead exemptions). Property taxes typically increase over time as home values appreciate and local budgets grow. Understanding your property tax obligation is crucial for long-term financial planning and comparing homes in different areas.`;
    };
